...
static void get_tick_params(lv_obj_t * obj, uint16_t i, int32_t * value_of_line_out,
                            lv_coord_t * line_width_out, lv_color_t * line_color_out);
static void calc_label_coord(lv_obj_t * obj, lv_point_t * p_center,
                             lv_coord_t r_in_major, bool is_hor, int32_t angle_upscale,
                             lv_point_t label_size, lv_point_t tick_start,
                             lv_point_t tick_end, lv_area_t * coord_out);
static void calc_start_and_end(lv_obj_t * obj, bool major, bool is_hor, uint16_t i,
...
static void get_min_scale(lv_obj_t * obj, int32_t * min_out, int32_t * max_out);
static bool lv_meter_is_hor(lv_obj_t * obj);
static uint16_t max_tick_length(lv_obj_t * obj);
...
void lv_meter_set_needle_side(lv_obj_t * obj, bool toggle_side)
{
    lv_meter_t * meter = (lv_meter_t *)obj;

    if(meter) {
        meter->scale.toggle_needle_side = toggle_side;
    }
}
...
static void get_min_scale(lv_obj_t * obj, int32_t * min_out, int32_t * max_out)
{
    ...
    if(base_dir == LV_BASE_DIR_RTL) {
        *min_out = meter->scale.max;
        *max_out = meter->scale.min;
    }
    else if(base_dir == LV_BASE_DIR_LTR) {
        *min_out = meter->scale.min;
        *max_out = meter->scale.max;
    }
}
...
static uint16_t max_tick_length(lv_obj_t * obj)
{
    ...
    return meter->scale.tick_major_length > meter->scale.tick_length ?
           meter->scale.tick_major_length : meter->scale.tick_length;
}
...
static void draw_arcs(lv_obj_t * obj, lv_draw_ctx_t * draw_ctx, const lv_area_t * scale_area)
{
    ...
    if(meter->mode == LV_METER_INDICATOR_MODE_ROUND) {
        ...
        _LV_LL_READ_BACK(&meter->indicator_ll, indic) {
            ...
            int32_t start_angle = lv_map(indic->start_value, meter->scale.min,
                                         meter->scale.max, meter->scale.rotation,
                                         meter->scale.rotation + meter->scale.angle_range);
            int32_t end_angle = lv_map(indic->end_value, meter->scale.min,
                                       meter->scale.max, meter->scale.rotation,
                                       meter->scale.rotation + meter->scale.angle_range);
        ...
        _LV_LL_READ_BACK(&meter->indicator_ll, indic) {
            ...
            max_tick_len = max_tick_length(obj);

            get_min_scale(obj, &scale_min, &scale_max);
            ...
            if(is_hor) {
                ...
                line_end.x = scale_area->x1 +  lv_map(indic->end_value, scale_min,
                                                      scale_max, 0, scale_len);
                if(meter->scale.tick_major_width) {
                    line_start.x -= meter->scale.tick_major_width / 2;
                    line_end.x += meter->scale.tick_major_width / 2;
                }
            }
            else {
                ...
                if(meter->scale.tick_major_width) {
                    line_start.y -= meter->scale.tick_major_width / 2;
                    line_end.y += meter->scale.tick_major_width / 2;
                }
            }
                               ...
                               lv_point_t * tick_start, lv_point_t * tick_end)
{
    ...
    if(is_hor) {
        tick_start->x = scale_area->x1 +
                        lv_area_get_width(scale_area) * i / (meter->scale.tick_cnt - 1);
        ...
        if(major) {
            ...
            tick_end->y = tick_start->y + meter->scale.tick_length;
        }
    }
    else {
        tick_start->x = scale_area->x1 +
        ...
        tick_start->y = scale_area->y1 +
                        lv_area_get_height(scale_area) * i / (meter->scale.tick_cnt - 1);
        ...
        if(major) {
            ...
            tick_end->x = tick_start->x + meter->scale.tick_length;
        }
    }
}

static void calc_label_coord(lv_obj_t * obj, lv_point_t * p_center, lv_coord_t r_in_major,
                             bool is_hor, int32_t angle_upscale, lv_point_t label_size,
                             lv_point_t tick_start, lv_point_t tick_end,
                             lv_area_t * coord_out)
{
    lv_point_t p;
    ...
    if(meter->mode == LV_METER_INDICATOR_MODE_ROUND) {
        ...
        if(is_hor) {
            ...
            if(meter->scale.toggle_needle_side) {
                p.y = tick_end.y + meter->scale.label_gap;
            }
            else {
                p.y = tick_start.y - meter->scale.label_gap;
            }
        }
        else {
            p.y = tick_start.y;
            /*Toggle labels side if need*/
            if(meter->scale.toggle_needle_side) {
                p.x = tick_end.x + meter->scale.label_gap;
            }
            else {
                p.x = tick_start.x - meter->scale.label_gap;
            }
        }
    }
                            ...
                            lv_coord_t * line_width_out, lv_color_t * line_color_out)
{
    ...
    int32_t value_of_line = lv_map(i, 0,  meter->scale.tick_cnt - 1, scale_min, scale_max);
            ...
            if(indic->type_data.scale_lines.color_start.full ==
                ...
                *line_color_out = indic->type_data.scale_lines.color_start;
                ...
                if(indic->type_data.scale_lines.local_grad) {
                    ...
                    ratio = lv_map(value_of_line,  meter->scale.min,
                                   meter->scale.max, LV_OPA_TRANSP, LV_OPA_COVER);
                }
...
static void draw_ticks_and_labels(lv_obj_t * obj, lv_draw_ctx_t * draw_ctx, const lv_area_t * scale_area)
{
    ...
    if(meter->mode == LV_METER_INDICATOR_MODE_ROUND) {
        r_in_minor = r_out - meter->scale.tick_length;
        r_in_major = r_out -  meter->scale.tick_major_length;
    ...
    if(meter->scale.tick_major_nth) {
        minor_cnt =  meter->scale.tick_major_nth - 1;
        max_tick_len =  meter->scale.tick_major_length;
    }
    else {
        minor_cnt = 0xFFFF;
        max_tick_len =  meter->scale.tick_length;
    }
    ...
    for(i = 0; i < meter->scale.tick_cnt; i++) {
        ...
        if(meter->mode == LV_METER_INDICATOR_MODE_LINEAR) {
            calc_start_and_end(obj, major, is_hor, i, max_tick_len, scale_area,
                               &tick_start, &tick_end);
        }

        line_dsc.color = major ? meter->scale.tick_major_color : meter->scale.tick_color;
        line_dsc.width = major ? meter->scale.tick_major_width : meter->scale.tick_width;

        int32_t angle_upscale = 0;
        lv_point_t p_outer = {0};

        get_tick_params(obj, i, &part_draw_dsc.value, &line_dsc.width,
                        &line_dsc.color);

        if(meter->mode == LV_METER_INDICATOR_MODE_ROUND) {
            angle_upscale = ((i * meter->scale.angle_range) * 10) / (meter->scale.tick_cnt - 1) +
                            meter->scale.rotation * 10;
        ...
        if(major) {
            ...
            calc_label_coord(obj, &p_center, r_in_major, is_hor, angle_upscale,
                             label_size, tick_start, tick_end, &label_cord);
        ...
        if(indic->type == LV_METER_INDICATOR_TYPE_NEEDLE_LINE ||
           indic->type == LV_METER_INDICATOR_TYPE_NEEDLE_TRIANGLE) {
            ...
            if(meter->mode == LV_METER_INDICATOR_MODE_ROUND) {
                ...
                int32_t angle;
                lv_coord_t r_out;
                angle = lv_map(indic->end_value, meter->scale.min,
                               meter->scale.max, meter->scale.rotation,
                               meter->scale.rotation + meter->scale.angle_range);
                r_out = r_edge + meter->scale.r_mod + indic->type_data.needle_line.r_mod;
            ...
            int32_t angle = lv_map(indic->end_value, meter->scale.min,
                                   meter->scale.max, meter->scale.rotation,
                                   meter->scale.rotation + meter->scale.angle_range);
...
static void inv_arc(lv_obj_t * obj, lv_meter_indicator_t * indic, int32_t old_value, int32_t new_value)
{
    ...
    if(meter->mode == LV_METER_INDICATOR_MODE_ROUND) {
        ...
        int32_t start_angle = lv_map(old_value, meter->scale.min, meter->scale.max,
                                     meter->scale.rotation,
                                     meter->scale.angle_range + meter->scale.rotation);
        int32_t end_angle = lv_map(new_value, meter->scale.min, meter->scale.max,
                                   meter->scale.rotation,
                                   meter-> scale.angle_range + meter->scale.rotation);
...
static void inv_line(lv_obj_t * obj, lv_meter_indicator_t * indic, int32_t value)
{
    ...
    if(indic->type == LV_METER_INDICATOR_TYPE_NEEDLE_LINE) {
        int32_t angle = lv_map(value, meter->scale.min, meter->scale.max,
                               meter->scale.rotation, meter->scale.rotation + meter->scale.angle_range);
