...
enum {
    LV_METER_INDICATOR_FORM_ROUND,
    LV_METER_INDICATOR_FORM_LINEAR,
};
typedef uint8_t lv_meter_indicator_form_t;
...
typedef struct {
    ...
    lv_meter_indicator_form_t form;
} lv_meter_t;
...
/**
 * Set indicator form. By default for is 'LV_METER_INDICATOR_FORM_ROUND'
 * For the classical 'LV_METER_INDICATOR_FORM_ROUND' meter
 * height and width of the 'obj' must be the same!
 * @param obj           pointer to a meter object
 * @param form          type of meter form
 */
void lv_meter_set_indicator_form(lv_obj_t * obj, lv_meter_indicator_form_t form);

/**
 * Set scale direction. Usable for vertical meter.
 * By default scale is from left to right(horizontal meter)
 * and top to bottom(vertical meter)
 * @param scale         scale
 * @param reverse_is_on enable reverse scale
 */
