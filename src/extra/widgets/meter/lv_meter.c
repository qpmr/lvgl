...
static void calc_label_coord(lv_meter_indicator_form_t form, lv_meter_scale_t * scale,
...
lv_obj_t * lv_meter_create(lv_obj_t * parent)
{
    ...
    /* By default form is classical round meter */
    lv_meter_t * meter = (lv_meter_t *)obj;
    meter->form = LV_METER_INDICATOR_FORM_ROUND;
                                                ...
                                                lv_color_t color, int16_t r_mod)
{
    ...
    if(meter->form == LV_METER_INDICATOR_FORM_ROUND) {
...
void lv_meter_set_indicator_form(lv_obj_t * obj, lv_meter_indicator_form_t form)
{
    LV_ASSERT_OBJ(obj, MY_CLASS);
    lv_meter_t * meter = (lv_meter_t *)obj;
    meter->form = form;

    /* Adjust default style for the Linear form meter */
    if(form == LV_METER_INDICATOR_FORM_ROUND) {
...
void lv_meter_set_direction(lv_meter_scale_t * scale, bool reverse_is_on)
{
    if(scale) {
        scale->reverse = reverse_is_on;
    }
}
...
static void draw_ticks_and_labels(lv_obj_t * obj, lv_draw_ctx_t * draw_ctx, const lv_area_t * scale_area)
{
    ...
    _LV_LL_READ_BACK(&meter->scale_ll, scale) {
        ...
        lv_coord_t r_in_minor;
        lv_coord_t r_in_major;
        lv_area_t area_inner_minor;
        lv_area_t area_inner_major;
        lv_area_t area_outer;
        int16_t outer_mask_id;
        ...
        for(i = 0; i < scale->tick_cnt; i++) {
            ...
            if(meter->form == LV_METER_INDICATOR_FORM_LINEAR) {
            ...
            if(major) {
                ...
                calc_label_coord(meter->form, scale, &p_center, r_in_major, is_hor,
        ...
        if(indic->type == LV_METER_INDICATOR_TYPE_NEEDLE_LINE ||
           indic->type == LV_METER_INDICATOR_TYPE_NEEDLE_TRIANGLE) {
            ...
            if(meter->form == LV_METER_INDICATOR_FORM_ROUND) {
                ...
                uint32_t max_tick_len;
                lv_point_t tick_center, tick_end;
...
static void inv_arc(lv_obj_t * obj, lv_meter_indicator_t * indic, int32_t old_value, int32_t new_value)
{
    ...
    if(meter->form == LV_METER_INDICATOR_FORM_ROUND) {
        ...
        if(indic->scale->reverse) {
            scale_min = indic->scale->max;
            scale_max = indic->scale->min;
        }
        else {
            scale_min = indic->scale->min;
            scale_max = indic->scale->max;
        }
