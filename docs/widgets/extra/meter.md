...
Meter widget can have a linear (vertical/horizontal) or round form.
By default, the form is round, but changing is simple: `lv_meter_set_indicator_form(meter, LV_METER_INDICATOR_FORM_LINEAR)`.
If the form is `LV_METER_INDICATOR_FORM_LINEAR`, the widget's orientation depends on it's size:
if width >= height - horizontal, otherwise - vertical.

Linear form has additional options:
- `lv_meter_set_direction(scale, reverse_is_on)` - for changing scale direction. By default horizontal meter has scale direction from
